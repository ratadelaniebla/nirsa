...
public class BarcoDescargaReaperturaCierreWindow extends GenericForwardComposer {
	...
	Integer estado;
	private Listbox lbxBarcoAperturado,lbxEstivaCubas,lbxBarcoCuba ;

	private Textbox txtRegistro,txtOrigen,txtFechaDescarga ;
...
//		}
	...
	public void obtenerRegistro()
	{
		try {
			...
			List<BarcoDescarga> listaBacroDescarga = new BarcoDescargaDAOJpaImpl().getBarcoDescargaporFechaTodos(td,th);
	...
	public void onSelect$lbxBarcoAperturado() throws InterruptedException{
		...
		if (lbxBarcoAperturado.getSelectedIndex()>0) {
			...
			if (barcoDescarga.getEstado()==0 ){
				chkApertura.setChecked(false);
				lblc.setValue("Descarga Cerrada");
			}else
			{
				chkApertura.setChecked(true);
				lblc.setValue("Descarga Aperturada");
			}
	...
	public void onCheck$chkApertura() throws InterruptedException{
	...
	{	
		...
		if(barcoDescarga==null)
		{
			Sistema.mensaje("No puede grabar. Debe elegir la Descarga");
			return;
		}
		if(chkApertura.isChecked())
		{
		...
		else
		{
			...
			barcoDescarga.setEstado(estado);
			//TODO
