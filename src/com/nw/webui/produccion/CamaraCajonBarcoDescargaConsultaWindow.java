			...
			{	if (lbxCajonesDisponibles.getSelectedIndex() > 0)
				{
					...
					if(camaraCajon.getEstado() == 7)
					{
						...
						if (pdlc == null)
						{
							...
							//Cajón fue usado en una producción	
							{
								...
								mensaje = mensaje +" Usado en la producción del :"+prod;
					...
					{
						Sistema.mensaje("ERROR.- Esta opción solo es disponible para cajones con estado de Devolucion al proveedor para poder habilitar");
					}
				}
			}
			else
			{
				Sistema.mensaje("Debe buscar el cajón devuelto al proveedor y luego confirmar la habilitación");
				return;
			}
		}
		else
		{
			Sistema.mensaje("Debe confirmar la habilitación del cajón por Devolución al proveedor");
		}
		//cajonconsultado.
	}	
	...
	public void onClick$btnConfirmar() throws ParseException
	{
		if (ChkConfirmar.isChecked())
		{
			...
			{	if (lbxCajonesDisponibles.getSelectedIndex() > 0)
				{
					ProduccionDetalleLoteCajon pdlc = new ProduccionDetalleLoteCajon();
					//Verifica sies un cajón con estado en venta
					...
					{
						Sistema.mensaje("ERROR.- Esta opción solo es disponible para cajones con estado en venta que se desean habilitar");
					}
				}
			}
			else
			{
				Sistema.mensaje("Debe buscar el cajón vendido y luego confirmar la habilitación");
				return;
			}
		}
		else
		{
			Sistema.mensaje("Debe confirmar la habilitación del cajón Vendido");
		}
		//cajonconsultado.
	}	
	...
	public void onClick$btnConfirmare() throws ParseException
	{
		if (ChkConfirmar.isChecked())
		{
			...
			{	if (lbxCajonesDisponibles.getSelectedIndex() > 0)
				{
					ProduccionDetalleLoteCajon pdlc = new ProduccionDetalleLoteCajon();
					//Verifica si es un cajón con estado eliminado
					...
					{
						Sistema.mensaje("ERROR.- Esta opción solo es disponible para cajones con estado eliminado que se desean habilitar");
					}
				}
			}
			else
		...
		{
			Sistema.mensaje("Debe confirmar la habilitación del cajón Eliminado");
		}
		//cajonconsultado.
	}	
...
//		}
		...
		if (txtCajon.getText().trim().isEmpty() == false)
		{
			...
			if (cajonconsultado==null) {
				Sistema.mensaje("No exite cajón que este regitrado en los ultimos 100 movimientos");
				return;
			}
		}
		...
		if (txtCajon.getText().trim().isEmpty() == false)
		{
			 if(res == 0)
			 {
				Sistema.mensaje("No se encontraron cajones con la parámetros  indicados");
...
//			{
//				Sistema.mensaje("Debe ingresar el peso neto para el nuevo cajón");
