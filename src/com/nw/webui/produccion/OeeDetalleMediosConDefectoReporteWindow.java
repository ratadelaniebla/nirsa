 ...
 *
 *  @author crholguin on 31/10/2017.
 *  Checkin Date   : 	 01/11/2017.
 *
 *  @Requerimiento : R21
 *  
 *  @Objetivo      : Reporte de Medios con defecto por proceso
 *             
 *  @Nota          :               
 *
 */
public class OeeDetalleMediosConDefectoReporteWindow extends GenericNw {
 
	private static final long serialVersionUID = -6907856294204269772L;
	
	OeeDefecto oeeDefecto;
	Proceso    proceso;
	OeeParada  parada;
	Turno 	   turno;
	...
	public void cargarListados(){
		//List<OeeDefecto> 			  oeeDefectos   =   new OeeDefectoDAOJpaImpl().getAll();
		List<Proceso> 				  procesos 		= 	new ProcesoDAOJpaImpl().getProcesosMP();
		
		//cargarListbox(new OeeDefecto() , oeeDefectos   ,  lbxDefecto   , "OeeDefecto" );
	...
	public void onClick$btnPdf() throws InterruptedException, SuspendNotAllowedException, JRException {
		...
		Map<String,Object> parameters = new HashMap<String,Object>();
		 ...
		 doImprimir("OeeDetalleMediosConDefectoReporteBi.jasper", parameters );
	...
	public void onClick$btnExcel() throws InterruptedException, FileNotFoundException, JRException {
	   ...
	   String logo = "logo_nirsa.gif";
		 ...
		 parameters.put("fecha_proceso", dteFechaInicial.getValue());
		 parameters.put("idproceso", proceso.getIdproceso());
		 new ReporteExcel().generarExcelJasper("OeeDetalleMediosConDefectoReporteBiExcel", "OeeDetalleMediosConDefectoReporteBiExcel.jasper", logo, parameters);
		...
		if (turno != null) {
			 if (pac!=null) {
				 ...
				 parameters.put("proceso", proceso.getDescripcionproceso());
				 parameters.put("fecha_proceso", pac.getFechaapertura());
				 new ReporteExcel().generarExcelJasper("OeeDetalleMediosConDefectoReporteBiExcel", "OeeDetalleParadaReporteBi.jasper", logo, parameters);
			 }else{
				 Sistema.mensaje("No existe proceso apertura para el turno y la fecha  escogida ");
			 }
		}else{
